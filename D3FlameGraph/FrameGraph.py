...
class StackFrame:
    ...
    def __init__(self, parent, name, frame_map):
        ...
        if parent:
            ...
            self._id.update(name.encode(StackFrame.encode))
    ...
    def NextId(self, name):
        ret = self._id.copy()
        ret.update(name.encode(StackFrame.encode))
        return ret
    ...
    def Print(self):
        ...
        buf.append(u"{\n")
        buf.append(u'"name":"%s"' % self._name)
        if self._children:
            buf.append(u',\n"children":[')
            sp = u""
        ...
        buf.append(u',"value":%d}\n' % value)
        if value:
            return value, u''.join(buf)
        return value, None
...
class FrameGraph:
    ...
    def Update(self, size):
        self._cursor.Update(size)

    def ToHTML(self):
        _v, _s = self._root.Print()
        return TEMPLATE.replace(u'$$WHOISYOURDADDY$$',_s).replace('$$$WHHHHHATTTTT$$$', self._what)
...
class Matcher:
    ...
    def match(self, txt):
        matched = self.ptn.match(txt)
        if not matched:
            return None
        ret = Matcher.Object()
        for k, v in matched.groupdict().items():
            if k[0] == 'd':
                ret.__setattr__(k, int(v))
                ret._d[k] = int(v)
            elif k[0] == 'f':
                ret.__setattr__(k, float(v))
                ret._d[k] = float(v)
            elif k[0] == 'x':
                ret.__setattr__(k, int(v, 16))
                ret._d[k] = int(v, 16)
            elif k[0] == 's':
                ret.__setattr__(k, v)
                ret._d[k] = v
